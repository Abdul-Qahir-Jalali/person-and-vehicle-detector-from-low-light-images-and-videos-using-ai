...
def verify(path):
    doc = Document(path)
    forbidden_words = ["deepfake", "audio", "elevenlabs", "wav2vec", "speech", "voice", "urdu", "fake", "sound", "mp3", "wav", "waveform", "hz", "vocals", "audio samples"]
    found = []
        ...
        for block in iter_block_items(parent):
            if isinstance(block, Paragraph):
                ...
                for w in forbidden_words:
                    # Note: "hz" might be a substring in other words, so check boundaries
                    import re
