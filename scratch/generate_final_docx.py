...
text_replacements = [
    ...
    ("DeepFake Audio Detection", "Robust Object Detection System"),
    ("deepfake audio detection", "robust object detection system"),
    ("Deepfake Audio Detection", "Robust Object Detection System"),
    ("Audio", "Video"),
    ("audio", "video"),
...
def process_document(input_path, output_path):
    ...
    for p in doc.paragraphs:
        ...
        # 1. Check if this is a heading we should replace
        is_replaced_heading = False
        for old_h, new_h in heading_replacements.items():
            if txt == old_h or txt.endswith(old_h):
                replace_run_text(p, txt.replace(old_h, new_h))
                current_section = new_h
                paragraphs_in_section = 0
                is_replaced_heading = True
                break
                
        if is_replaced_heading:
            continue
            
        # 2. Check if this matches an exact key in new_content (e.g. "ABSTRACT", "1.1 Problem Statement and Motivation")
        if txt in new_content or txt.split(" ", 1)[-1] in new_content or (len(txt.split(" ")) > 1 and txt.split(" ", 1)[-1] in heading_replacements.values()):
            if txt in new_content:
                current_section = txt
            else:
                # E.g. "1.1 Problem Statement and Motivation" where maybe the txt is "1.1 Problem Statement and Motivation"
                # If txt is exactly a key in new_content
                found = False
                for k in new_content.keys():
                    if txt == k or txt.endswith(k) or txt.replace(" ", "") == k.replace(" ", ""):
                        current_section = k
                        found = True
                        break
                if found:
            ...
            # Apply text replacements on the heading itself (like "ABSTRACT")
            new_txt = txt
            for old, new in text_replacements:
                new_txt = new_txt.replace(old, new)
            if new_txt != txt:
                replace_run_text(p, new_txt)
            continue
            
        # 3. If we are inside a section, replace its content paragraphs
        # A content paragraph is usually a normal paragraph, not a heading, and has length > 20
        if current_section and current_section in new_content and len(txt) > 20:
            if paragraphs_in_section < len(new_content[current_section]):
                replace_run_text(p, new_content[current_section][paragraphs_in_section])
                paragraphs_in_section += 1
            else:
                # Extra old paragraphs in this section, clear them
                replace_run_text(p, "")
            continue
            
        # 4. Global replacements for anything else (Title page, TOC, captions)
        if len(txt) > 0:
            new_txt = txt
            for old, new in text_replacements:
                new_txt = new_txt.replace(old, new)
            if new_txt != txt:
                # We replace preserving run 0 style to keep TOC formatting somewhat intact
                replace_run_text(p, new_txt)
